...
namespace libfreebsdnet::types {
  ...
  Address::Address(const std::string &ip, int prefixLen)
    : ip_(ip), prefixLen_(prefixLen), family_(Family::UNKNOWN), valid_(false) {
  ...
  int Address::getPrefixLength() const {
    return prefixLen_;
  }

  Address::Family Address::getFamily() const {
    return family_;
  }
  ...
  bool Address::parseAddress(const std::string &addressString, 
  ...
  std::string Address::prefixToNetmask(int prefixLen, Family family) {
    if (family == Family::IPv4) {
      ...
      if (prefixLen < 0 || prefixLen > 128) return "";
  ...
  std::string Address::calculateBroadcast(const std::string &ip, 
