...
namespace libfreebsdnet::interface {
  ...
  Manager::Manager() {
    ...
    if (socket_fd < 0) {
      throw std::runtime_error("Failed to create socket for interface operations");
    }
  }
  ...
  std::vector<std::unique_ptr<Interface>> Manager::getInterfaces() const {
    ...
    for (struct ifaddrs *ifa = ifaddrs_ptr; ifa != nullptr; ifa = ifa->ifa_next) {
      ...
      if (ifa->ifa_name && ifa->ifa_addr && ifa->ifa_addr->sa_family == AF_LINK) {
        ...
        if (ioctl(socket_fd, SIOCGIFFLAGS, &ifr) == 0) {
          ...
          if (name.substr(0, 6) == "bridge") {
            ...
            interface = std::make_unique<EpairInterface>(ifa->ifa_name, index, ifr.ifr_flags);
          } else if (name.substr(0, 4) == "vlan") {
            interface = std::make_unique<VlanInterface>(ifa->ifa_name, index, ifr.ifr_flags);
          } else if (name.substr(0, 6) == "l2vlan") {
            interface = std::make_unique<L2VlanInterface>(ifa->ifa_name, index, ifr.ifr_flags);
          } else if (name.substr(0, 5) == "pfsync") {
            interface = std::make_unique<PfsyncInterface>(ifa->ifa_name, index, ifr.ifr_flags);
          } else if (name.substr(0, 5) == "pflog") {
            interface = std::make_unique<PflogInterface>(ifa->ifa_name, index, ifr.ifr_flags);
          } else if (name.substr(0, 4) == "carp") {
            interface = std::make_unique<CarpInterface>(ifa->ifa_name, index, ifr.ifr_flags);
          } else {
            // Default to Ethernet for everything else
            interface = std::make_unique<EthernetInterface>(ifa->ifa_name, index, ifr.ifr_flags);
          }
  ...
  std::unique_ptr<Interface> Manager::getInterface(const std::string &name) const {
    struct ifaddrs *ifaddrs_ptr;
    if (getifaddrs(&ifaddrs_ptr) == -1) {
      return nullptr;
    }
    ...
    for (struct ifaddrs *ifa = ifaddrs_ptr; ifa != nullptr; ifa = ifa->ifa_next) {
      if (ifa->ifa_name && std::string(ifa->ifa_name) == name && ifa->ifa_addr) {
        unsigned int index = if_nametoindex(ifa->ifa_name);
  ...
  std::unique_ptr<Interface> Manager::createInterface(const std::string &name, unsigned int index, int flags) {
    // Get the actual interface type from the system using sockaddr_dl
    struct ifaddrs *ifaddrs_ptr;
    std::unique_ptr<Interface> interface = nullptr;
      ...
      for (struct ifaddrs *ifa = ifaddrs_ptr; ifa != nullptr; ifa = ifa->ifa_next) {
        if (ifa->ifa_name && std::string(ifa->ifa_name) == name) {
          ...
          if (ifa->ifa_addr && ifa->ifa_addr->sa_family == AF_LINK) {
            ...
            // Create the correct interface class based on sdl_type and ioctl capabilities
            // Some interfaces (bridge, lagg) present as Ethernet at link layer
            // but need to be detected by their specific ioctl support
            std::string if_name(ifa->ifa_name);
            ...
            if (ioctl(socket_fd, SIOCSLAGG, &lagg_req) == 0 || errno == EINVAL) {
              ...
              if (ioctl(socket_fd, SIOCGDRVSPEC, &ifd) == 0 || errno == EINVAL) {
                // Interface supports bridge ioctl
                interface = std::make_unique<BridgeInterface>(name, index, flags);
              } else {
                ...
                case IFT_ETHER:
                  ...
                  break;
                case IFT_LOOP:
                  interface = std::make_unique<LoopbackInterface>(name, index, flags);
                  break;
                case IFT_BRIDGE:
                  interface = std::make_unique<BridgeInterface>(name, index, flags);
                  break;
                case IFT_IEEE80211:
                  interface = std::make_unique<WirelessInterface>(name, index, flags);
                  break;
                case IFT_L2VLAN:
                  interface = std::make_unique<L2VlanInterface>(name, index, flags);
                  break;
                case 0x88: // IFT_L3IPVLAN (EPAIR)
                  interface = std::make_unique<EpairInterface>(name, index, flags);
                  break;
  ...
  std::unique_ptr<Interface> Manager::createInterface(const std::string &name, unsigned int index, int flags, InterfaceType type) {
    switch (type) {
