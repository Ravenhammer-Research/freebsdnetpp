 ...
 * @details Implementation of GIF interface show-related command handlers for
 * the net tool
 *
...
namespace net {
  ...
  bool
  NetTool::handleShowInterfaceTypeGif(const std::vector<std::string> &args) {
    ...
    std::vector<std::string> headers = {"Interface", "Status", "Local",
                                        "Remote", "FIB"};

    for (const auto &interface : gifInterfaces) {
      auto gifIface = dynamic_cast<libfreebsdnet::interface::GifInterface *>(
          interface.get());
      if (!gifIface) {
        continue;
      }
